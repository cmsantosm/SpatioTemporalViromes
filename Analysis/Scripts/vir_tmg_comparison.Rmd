...
get_wilcox_ts <- function(x) {
  wilcox.test(x$BULK, x$VFD, paired = TRUE, alternative  = "two.sided") %>% 
  broom::tidy()
}  

wilcox.test <- phy.nest %>% 
  mutate(Wilcox_ts = map(data, get_wilcox_ts)) %>% 
  mutate(Wilcox_greater = map(data, get_wilcox_greater)) %>% 
  mutate(Wilcox_less = map(data, get_wilcox_less)) 

wt.p <- rbind(unnest(wilcox.test, Wilcox_ts), unnest(wilcox.test, Wilcox_greater), unnest(wilcox.test, Wilcox_less)) %>% 
  group_by(alternative) %>% 
  mutate(p.adjusted = p.adjust(p.value)) %>% 
  filter(p.adjusted < 0.05) %>% 
  filter(alternative != "two.sided") %>%
  ungroup() %>% 
  mutate(Enrichment = ifelse(alternative == "greater", "Total MG", "Virome")) %>% 
  ggplot(aes(reorder(Phylum, -p.adjusted), -log10(p.adjusted), fill = Enrichment)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c(RColorBrewer::brewer.pal(9, "Reds")[6],RColorBrewer::brewer.pal(9, "Blues")[6])) +
  xlab("Phylum") +
  ylab("Adjusted P-val (-log10)") +
  coord_flip() +
  theme_light() +
  theme(text = element_text(size = 15),
...
```{r}
### 992:621
bottom <- cowplot::plot_grid(wt.p, rrcurve.p, nrow = 1, labels = c("B", "C"), rel_widths = c(3,2),label_size = 20)
...
limits <- map.stats %>% 
  ...
  summarise(sdMR = sd(Mapped75),
            meanMR = mean(Mapped75),
            sdOTUs = sd(nOTUs),
            meanOTUs = mean(nOTUs)) %>% 
  ungroup() %>% 
  mutate(maxMR = meanMR + 2*sdMR,
         minMR = meanMR - 2*sdMR,
         maxOTUs = meanOTUs + 2*sdOTUs,
         minOTUs = meanOTUs - 2*sdOTUs)
